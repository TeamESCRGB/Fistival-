...
namespace Coordinator.Hands
{
    public class RootShooterHand : HandCoordinatorBase, IPointerMovementInputHandler
    {
...
#endif
            _movLocker = GetComponentInParent<IMovementLockable>();
#if UNITY_EDITOR
            if (_movLocker == null)
            {
                Debug.LogError($"{transform.parent.name} 에 IMovementLockable이 없습니다");
        ...
        public void DeInit()
        {
            _attackBox.SetParent(transform);//나중에 ui로 옮기면 바꾸고, 옮기면 그대로.
        }
...
패닝샷 기간동안 잠갔다가, 패닝샷이 끝나거나, 스턴이 걸리면 락 해제
